...
const getCartData = catchAsync(async (req, res, next) => {
  const query = `SELECT azst_cart_id,product_title,azst_variant_id,price ,azst_quantity ,azst_customer_id,image_src,product_url_title,
                  ...
                  FROM  azst_cart_tbl
                  Left JOIN azst_sku_variant_info ON azst_cart_tbl.azst_variant_id = azst_sku_variant_info.id
                  left JOIN azst_products ON azst_cart_tbl.azst_product_id  = azst_products.id 
                  WHERE  azst_customer_id = ?  AND azst_cart_tbl.status =1 `;
