...
exports.addInvetroyLoation = catchAsync(async (req, res, next) => {
  ...
  const inveQuery = `INSERT INTO azst_inventory (inventory_id,inventory_name,
                        inventory_location,inventory_longitude,inventory_latitude,
                        inventory_address,inventory_mail,inventory_phone)
                    VALUES (?,?,?,?,?,?,?,?)`;
  const values = [
    inventoryId,
    inventoryName,
...
exports.getinventories = catchAsync(async (req, res, next) => {
  const invQuery = `SELECT inventory_id,inventory_name
                        FROM azst_inventory WHERE inventory_status = 1`;
  const result = await db(invQuery);
  res.status(200).json({ inventories: result });
});
...
exports.isInventoryExsit = catchAsync(async (req, res, next) => {
  ...
  const getbrand = `SELECT * FROM azst_inventory WHERE  inventory_id = ${inventoryId} AND inventory_status = 1`;
...
exports.updateInventory = catchAsync(async (req, res, next) => {
  ...
  const updateQuery = `UPDATE azst_inventory SET inventory_name =?,
                        inventory_location =?,inventory_longitude =?,inventory_latitude =?,
                        inventory_address =?,inventory_mail=?,inventory_phone=?
                       WHERE inventory_id = ?`;
  const values = [
...
exports.deleteInventory = catchAsync(async (req, res, next) => {
  ...
  const deletecollection =
    'UPDATE azst_inventory SET inventory_status = 0, updatedby=? WHERE inventory_id = ? ';
